...
Deno.serve(async (req) => {
  ...
  try {
    const { customerName, customerPhone, comment, address, productName, category, brand } = await req.json();
    ...
    const safeAddress = address ? String(address).slice(0, 200).replace(/[<>&]/g, '') : '';
    ...
    const text = `🛒 <b>Новый заказ!</b>\n\n` +
      ...
      (safeAddress ? `📍 <b>Адрес:</b> ${safeAddress}\n` : '') +
